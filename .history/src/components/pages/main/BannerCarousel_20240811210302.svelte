  ...
  import type { MouseEventHandler, TouchEventHandler } from "svelte/elements";
    ...
    const adjustSlide = () => {
        ...
        if (positionDiff > firstImgWidth / 3) {
            let distanceToPrev = Math.abs(positionDiff - (firstImgWidth * currentSlide));
            console.log("prev")
        } else if (positionDiff < -firstImgWidth / 3) {
            if (currentSlide === slides.length - 1) return;
            let distanceToNext = Math.abs(positionDiff - (firstImgWidth * (currentSlide + 1)));
            console.log("next", distanceToNext)
        } else {
            console.log("stay")
        }
        console.log('currentScroll', carousel.scrollLeft)
        console.log('currentSlide', currentSlide)
        currentSlide = Math.round(carousel.scrollLeft / firstImgWidth);
    }
...
<button on:click={() => {carousel.scrollLeft -= firstImgWidth}}>&lt;</button>
<button on:click={() => {carousel.scrollLeft += firstImgWidth}}>&gt;</button>
