<script lang="ts">
    ...
    const navOptions: { name: string, href: PageIndex, state: 'show' | 'hide', selected: ComponentType, unSelected: ComponentType }[] = [
        {
            name: 'Home',
            href: PageIndex.HOME,
            state: "show",
    ...
    function navigateTo(page: PageIndex) {
        ...
        for (const option in navOptions) {
            if (navOptions[option].href === page) {
                navOptions[option].state = "show";
            } else {
                navOptions[option].state = "hide";
            }
        }
    }
</script>
