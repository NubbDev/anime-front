...
<nav class="nav_menu">
    {#each navOptions as option}
        <a href={option.href} class={path === option.href ? "selected" : "unselected"}>
            {#if path === option.href} 
                    <svelte:component this={option.selected} name={option.name} class="selected"/> 
                    <span style="color: var(--icon-selected);">{option.name}</span>
                {:else}
                    <svelte:component this={option.unSelected} fill="white" name={option.name} class="unselected"/>
...
<style>
    ...
    :global(.nav_menu a.selected svg) {
        fill: gradient(var(--icon-selected), var(--icon-selected));
        width: 1.5rem;
        height: auto;
    }
