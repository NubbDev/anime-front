...
async fn get_trending(page: &str) -> Result<serde_json::Value, ()> {
    let json: serde_json::Value = match page.is_empty() {
        ...
        false => {
            let res = match page.parse::<i32>() {
                Ok(page) => ureq::get(format!("{}/anime/trending/{}", API_URL, page).as_str())
                    .call()
                    .expect("Failed to get data from API"),
                Err(_) => {
            ...
            let json: serde_json::Value = res.into_json().expect("Failed to parse JSON");
            json
        }
    };
    Ok(json)
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![greet, get_trending])
